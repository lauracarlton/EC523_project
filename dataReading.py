...
baseDir = '/Users/lauracarlton/Documents/EC523_project/child-mind-institute-detect-sleep-states/'
# file_path = '/Users/erynd/Library/CloudStorage/OneDrive-Personal/Grad School/Deep Learning/Project/train_series.parquet'
file_path = baseDir + 'train_series.parquet'
...
csv_file_path = baseDir + 'train_events.csv'
...
my understanding of the data:
    ...
    - series_id indicates a subject or consecutive run (unique identifier of each series)
    - step corresponds to one step in timestamp which has the actual time
    - anglez = angle arm relative to vertical axis of body 
    - enmo = euclidean norm minus one of accelerometer signals 
    train_events.csv
    - series_id corresponds to same run in train_series
    - night = onset/wakup pairs - consecutive events in the run, one per night
    - event = indicates whether it is the onset of sleep or wakeup 
    - step/timestamp - same as in train_series


pre-processing thoughts:
-> drop NaN
-> look at histograms of enmo and anglez
-> plot time vs enmo, anglez for a specific series id 
-> there are ways to reduce memory usage - could be useful if things are really slow
-> how do we use data between sleep and wakeup ? 
    this could have predictive power 
    when you merge tables what happens when 
'''
...
model.add(Dense(units=1, activation='softmax'))  #binary classification
